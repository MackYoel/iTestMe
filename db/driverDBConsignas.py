...
class Consignas():
	...
	def registrarConsigna(self,params):
		...
		self.db.execute("INSERT INTO consignas(id,consigna,tipo,dificultad,respuesta,aclaracion) \
					values (?,?,?,?,?,?)",(int(uid)+1, params[0],params[1],params[2],params[3],params[4]))
		self.db.commit()
		return True
	...
	def consignaAleatoria(self,tipo=None, dificultad=None):	
		if(dificultad is None or dificultad == "cualquiera"):
			obtenida = self.db.execute("SELECT * FROM consignas where tipo='%s'"%tipo)
		elif(dificultad is None and tipo is None):
			obtenida = self.db.execute("SELECT * FROM consignas")
		elif(tipo and dificultad):
			obtenida = self.db.execute("SELECT * FROM consignas where tipo='%s' and dificultad='%s'"%(tipo,dificultad))
		else:
			return False
		return list(random.choice(obtenida.fetchall()))
	
	def obtenerPorId(self, idConsigna):
		consultaID = self.db.execute("SELECT * FROM consignas where id='%s'"%idConsigna)
